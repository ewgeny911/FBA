...
INSERT INTO fbaEntityParser(ID,
    EntityID,
    Name,
    Brief,
    ParentID,
...
UPDATE fbaEntityParser SET Table_ID          = (SELECT t2.TableID     FROM fbaTable t2 WHERE t2.TableEntityID = fbaEntityParser.ID AND t2.Type = 1);
UPDATE fbaEntityParser SET Table_Name        = (SELECT t2.Name        FROM fbaTable t2 WHERE t2.TableEntityID = fbaEntityParser.ID AND t2.Type = 1);
UPDATE fbaEntityParser SET Table_IDFieldName = (SELECT t2.IDFieldName FROM fbaTable t2 WHERE t2.TableEntityID = fbaEntityParser.ID AND t2.Type = 1);
--select * from fbaEntityParser
...
UPDATE fbaEntityParent SET EnBrief2_TableName = (SELECT Table_Name FROM fbaEntityParser t1 WHERE fbaEntityParent.EnBrief2 = t1.Brief);
UPDATE fbaEntityParent SET EnBrief2_TableIDFieldName = (SELECT Table_IDFieldName FROM fbaEntityParser t1 WHERE fbaEntityParent.EnBrief2 = t1.Brief);
                                                                      
--Данные по таблице для сущности EnBrief1                               
UPDATE fbaEntityParent SET EnBrief1_TableName = (SELECT Table_Name FROM fbaEntityParser t1 WHERE fbaEntityParent.EnBrief2 = t1.Brief);
UPDATE fbaEntityParent SET EnBrief1_TableIDFieldName = (SELECT Table_IDFieldName FROM fbaEntityParser t1 WHERE fbaEntityParent.EnBrief2 = t1.Brief);      
--select * from fbaEntityParent ORDER BY enID;         
